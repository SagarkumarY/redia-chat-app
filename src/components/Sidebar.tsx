...
function Sidebar({ isCollapsed, users }: SidebarProps) {
  ...
  return (
    <div className=" relative flex flex-col h-full gap-4 p-2 data-[collapsed=true] max-h-full overflow-auto bg-background">
      {!isCollapsed && (
            ...
            <TooltipProvider key={idx}>
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  ...
                  >
                    <Avatar className=" my-1 flex justify-between items-center cursor-pointer">
                      <AvatarImage
            ...
            <Button
              ...
              className={cn(
                ...
                selectedUser?.email === user.email &&
                  "dark:bg-muted dark:text-white dark:hover:bg-muted dark:hover:text-gray-400 shrink"
              )}
              variant={"grey"}
              size={"xl"}
              onClick={() => {
      ...
      <div className="mt-auto">
        <div className="flex justify-between items-center gap-2 md:px-6 py-2">
          ...
          <LogoutLink>
            <LogInIcon size={22} cursor={"pointer"} />
          </LogoutLink>
        </div>
      </div>
    </div>
